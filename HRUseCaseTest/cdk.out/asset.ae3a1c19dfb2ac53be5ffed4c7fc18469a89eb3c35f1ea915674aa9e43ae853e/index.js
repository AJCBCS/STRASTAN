...
exports.handler = async (event) => {
  for (const record of event.Records) {
    ...
    const employees = await Promise.all(
      employeeQuery.Items.map(async (empItem) => {
        const base = unmarshall(empItem);
        const employeeId = base.employeeId || (base.PK ? base.PK.split("#")[1] : null);
        let fullName = "Unknown";
        let positionName = "Unknown";
        if (!employeeId) {
          console.warn("\u26A0\uFE0F Employee ID not found in base item:", base);
          return null;
        }
        try {
          const personalDataRes = await dbClient.send(new QueryCommand({
            TableName: PERSONNEL_TABLE,
          ...
          const allItems = personalDataRes.Items ? personalDataRes.Items.map(unmarshall) : [];
          const combined = allItems.reduce((acc, item) => ({ ...acc, ...item }), {});
          const firstName = combined.firstName ? decrypt(combined.firstName) : "";
          const lastName = combined.lastName ? decrypt(combined.lastName) : "";
          fullName = `${firstName} ${lastName}`.trim() || "Unknown";
          positionName = combined.role || "Unknown";
        } catch (err) {
          console.error(`\u274C Failed to fetch/decrypt info for employee ${employeeId}:`, err);
        }
        return {
          employeeId,
          fullName,
