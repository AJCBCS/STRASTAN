    ...
    const employees = await Promise.all(
      employeeQuery.Items.map(async (emp) => {
        ...
        try {
          ...
          console.error(`\u26A0\uFE0F Error fetching personal data for ${base.employeeId}:`, e);
        }
        if (base.positionId) {
          try {
            const posRes = await dbClient.send(new GetItemCommand({
              TableName: ORGANIZATIONAL_TABLE,
              Key: {
                PK: { S: `ORG#POSITION#${base.positionId}` },
                SK: { S: "METADATA" }
              }
            }));
            if (posRes.Item) {
            ...
            console.error(`\u26A0\uFE0F Error fetching position for ${base.employeeId}:`, e);
          }
        }
        return {
