...
exports.handler = async (event) => {
  ...
  try {
    const enrichedViewRes = await dbClient.send(new GetItemCommand({
      TableName: tableName,
      Key: {
    ...
    if (enrichedViewRes.Item) {
      const enriched = unmarshall(enrichedViewRes.Item);
      console.log(`\u2705 Returning enriched view for department ID: ${departmentId}`);
      return {
        statusCode: 200,
        body: JSON.stringify({ department: enriched })
      };
    }
    console.log(`\u26A0\uFE0F Enriched view not found. Falling back to metadata.`);
    const getParams = {
      TableName: tableName,
      Key: {
        PK: { S: `ORG#DEPARTMENT#${departmentId}` },
    ...
    try {
      ...
      console.log(`\u2705 Published department ${data.departmentId} to SNS`);
    } catch (snsError) {
    ...
    console.error("\u274C Error while retrieving department:", error);
    return {
      statusCode: 500,
      body: JSON.stringify({
